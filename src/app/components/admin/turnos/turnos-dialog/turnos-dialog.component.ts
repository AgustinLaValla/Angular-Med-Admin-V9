...
export class TurnosDialogComponent implements OnInit, OnDestroy {
  ...
  initDialog() {
    ...
    this.especialidadSubs$ = this.store.select(getEspecialidad).subscribe((especialidad: Especialidad) => {
      if (especialidad) {
        this.servicios = especialidad.servicios;
      }
    })

    this.miemboSubs$ = this.store.select(getMiembro).subscribe((miembro: Miembro) => {
      if (miembro) {
        this.miembro = miembro;
        this.new_turno.especialista = `${this.miembro.apellido} ${this.miembro.nombre}`;
    ...
    this.pacientSubs$ = this.store.select(getPacient).subscribe((pacient: Pacient) => {
      if (pacient) {
        ...
        if (nombre && apellido && id && obra_social && nacimiento && nacimiento_seconds) {
          ...
          this.new_turno.nacimiento_seconds = pacient.nacimiento_seconds
        }
      }

    });

    this.dialogCloseSubs$ = this.store.select(getCloseDialog).subscribe((close) => {
      if (close) {
  ...
  showDetails() {
    ...
    if (this.compare(this.new_turno.desde, this.new_turno.hasta)) return;
  ...
  compare(hora_desde: string, hora_hasta: string) {
    ...
    if (moment(hora_desde).clone().unix() >= moment(hora_hasta).clone().unix()) {
      Swal.fire({
        ...
        html: `<span><strong>¡La hora de inicio debe ser mayor a la hora de cierre!</strong></span><br>
               <u>Hora de Inicio:</u>${hora_desde}<br> 
               <u>Hora de cierre:</u>${hora_hasta}<br>`,
